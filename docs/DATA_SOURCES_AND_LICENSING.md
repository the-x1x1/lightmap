...
| `open-meteo-era5`                                                                  | Monthly climatology from ERA5 reanalysis (1940→)      | Same terms as `open-meteo` (archive endpoint; CC BY 4.0 free tier, non-commercial; commercial via `customer-archive-api`); ERA5 © Copernicus Climate Change Service | **Yes** — "Climatology from ERA5 reanalysis (Copernicus) via Open-Meteo.com" | Conditional until `OPEN_METEO_API_KEY` is set                        | Yes: one summary per 0.5° cell × month × year span, TTL 30 days                      | Derived monthly shares only | Included in the Open-Meteo plan                                                        | Any archive behind `ClimatologyProvider` (Meteostat, NOAA GHCN, ERA5 direct)                    |
...
| `fixture-geocoder`, `fixture-timezone`, `fixture` (weather), `fixture-climatology` | Deterministic development data                        | Proprietary test data                                                                                                                                               | n/a (labelled "not a forecast / not a live source")                          | **Development-only**                                                 | n/a                                                                                  | n/a                         | $0                                                                                     | n/a                                                                                             |
...
### Open-Meteo archive / ERA5 (`open-meteo-era5`)

Phase 7 climatology. `OpenMeteoClimatologyProvider` requests hourly `cloud_cover` and
`precipitation` for one calendar month of one year (≤ 744 rows) from `archive-api.open-meteo.com`
(or `customer-archive-api` with the key); `buildMonthlyClimatology` fetches ten years with
concurrency 3 and reduces them to shares of the five scenario classes over daylight hours. Only the
derived summary is stored (30-day cache per 0.5° cell); raw hours are discarded. ERA5 is a
reanalysis — a model estimate of the past — so the UI says "typical", never "it was", and the badge
reads "Climatology · not a forecast". Attribution names both Open-Meteo and Copernicus.
