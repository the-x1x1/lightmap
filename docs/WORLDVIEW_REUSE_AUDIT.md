...
**Rule applied:** the LightMap plan, §0A. *The destination architecture wins.*
...
no runtime dependency on WorldView. The rest of WorldView's map stack was used as a *reference*
...
- `git log --follow` on any file listed under *ported* shows it was authored in the LightMap
