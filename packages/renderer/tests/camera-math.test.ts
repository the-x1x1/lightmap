...
import { altitudeForBounds, altitudeToZoom, clampOrbit, normalizeHeadingDegrees, zoomToAltitudeM } from '../src/camera-math.ts';
...
describe('camera math (ported from WorldView view.ts / contract.ts)', () => {
  ...
  it('zoom ↔ altitude round-trips and is monotone', () => {
    for (const z of [2, 8, 12, 16]) expect(altitudeToZoom(zoomToAltitudeM(z, 21.4), 21.4)).toBeCloseTo(z, 6);
  ...
  it('frames bounds with a margin', () => {
    ...
    expect(altitudeForBounds({ west: 179, south: -1, east: -179, north: 1 })).toBeGreaterThan(200_000);
  });
  it('clamps orbit views', () => {
    expect(clampOrbit({ headingDeg: 400, pitchDeg: 10, rangeM: 1 })).toEqual({ headingDeg: 40, pitchDeg: -5, rangeM: 50 });
