...
export const profiles = pgTable('profiles', {
  ...
  defaultTimezoneBehavior: text('default_timezone_behavior', { enum: ['location', 'device'] }).notNull().default('location'),
...
export const viewpoints = pgTable('viewpoints', {
  ...
  weatherMode: text('weather_mode', { enum: ['FORECAST', 'EXTENDED_FORECAST', 'SCENARIO', 'RECENT_PAST', 'PAST'] }).notNull(),
  weatherScenario: text('weather_scenario'),
  previewSourceType: text('preview_source_type', { enum: ['REAL_REFERENCE', 'SIMULATED_LIGHTING', 'ESTIMATED_PREVIEW'] }).notNull(),
  ...timestamps,
}, (t) => [index('viewpoints_project_idx').on(t.projectId), index('viewpoints_user_idx').on(t.userId)]);

export const previewSnapshots = pgTable('preview_snapshots', {
  id: text('id').primaryKey(),
  viewpointId: text('viewpoint_id').notNull().references(() => viewpoints.id, { onDelete: 'cascade' }),
...
export const subscriptions = pgTable('subscriptions', {
  ...
  updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
}, (t) => [uniqueIndex('subscriptions_user_idx').on(t.userId), uniqueIndex('subscriptions_customer_idx').on(t.providerCustomerId), index('subscriptions_provider_sub_idx').on(t.providerSubscriptionId)]);
...
export const providerCache = pgTable('provider_cache', {
  ...
  createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
}, (t) => [primaryKey({ columns: [t.namespace, t.cacheKey] }), index('provider_cache_expires_idx').on(t.expiresAt)]);
...
export const auditEvents = pgTable('audit_events', {
  ...
  createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
}, (t) => [index('audit_events_user_idx').on(t.userId), index('audit_events_created_idx').on(t.createdAt)]);
