...
import type { GeocodingProvider, ImageryProvider, MapTileProvider, ProviderMeta, TerrainProvider, TimezoneProvider } from './types.ts';
...
export function createGeospatialProviders(env: Env, opts: RegistryOptions = {}): GeospatialProviders {
  const geocoder: GeocodingProvider =
    env.GEOCODER_PROVIDER === 'nominatim'
      ? new NominatimGeocoder({ userAgent: env.GEOCODER_USER_AGENT, ...(opts.fetchImpl ? { fetchImpl: opts.fetchImpl } : {}) })
      : new FixtureGeocoder();
  ...
  switch (env.TERRAIN_PROVIDER) {
    case 'reearth':
      ...
      break;
    case 'cesium-ion':
      terrain = env.CESIUM_ION_TOKEN ? new CesiumIonTerrainProvider() : new EllipsoidTerrainProvider();
      break;
    case 'ellipsoid':
    ...
    case 'xyz':
      basemap =
        env.IMAGERY_XYZ_URL && env.IMAGERY_XYZ_ATTRIBUTION
          ? new XyzBasemap({ url: env.IMAGERY_XYZ_URL, attribution: env.IMAGERY_XYZ_ATTRIBUTION, maxZoom: env.IMAGERY_XYZ_MAX_ZOOM })
...
export interface PublicProviderDescriptors {
  ...
  basemap: ReturnType<MapTileProvider['descriptor']> & { providerId: string; detailLevel: MapTileProvider['detailLevel']; isFixture: boolean };
  geocoderId: string;
...
export function publicDescriptors(p: GeospatialProviders): PublicProviderDescriptors {
  return {
    terrain: { ...p.terrain.descriptor(), providerId: p.terrain.meta.id, isFixture: p.terrain.meta.isFixture },
    basemap: { ...p.basemap.descriptor(), providerId: p.basemap.meta.id, detailLevel: p.basemap.detailLevel, isFixture: p.basemap.meta.isFixture },
    geocoderId: p.geocoder.meta.id,
    geocoderIsFixture: p.geocoder.meta.isFixture,
    referenceImagery: p.referenceImagery !== null,
    attributions: activeAttributions(p).map((m) => ({ id: m.id, attribution: m.attribution, ...(m.attributionUrl ? { attributionUrl: m.attributionUrl } : {}) })),
  };
}
