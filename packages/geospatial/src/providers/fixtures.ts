...
export const DEV_LOCATIONS: readonly DevLocation[] = [
  {
    id: 'kailua-beach',
    label: 'Kailua Beach, Hawaii, USA',
    point: { latitude: 21.397, longitude: -157.727, elevationM: 2 },
    timezone: 'Pacific/Honolulu',
    countryCode: 'US',
  },
  {
    id: 'grand-canyon',
    label: 'Grand Canyon, Mather Point, Arizona, USA',
    point: { latitude: 36.0617, longitude: -112.1078, elevationM: 2170 },
    timezone: 'America/Phoenix',
    countryCode: 'US',
  },
  {
    id: 'manhattan',
    label: 'Manhattan, New York, USA',
    point: { latitude: 40.7484, longitude: -73.9857, elevationM: 15 },
  ...
  },
  {
    ...
    point: { latitude: 51.5074, longitude: -0.1278, elevationM: 11 },
  ...
  },
  {
    ...
    point: { latitude: -33.8688, longitude: 151.2093, elevationM: 20 },
  ...
  },
  {
    id: 'tromso',
    label: 'Tromsø, Norway',
    point: { latitude: 69.6492, longitude: 18.9553, elevationM: 10 },
...
export class FixtureGeocoder implements GeocodingProvider {
  ...
  async search(query: string, opts?: { limit?: number }): Promise<Place[]> {
    ...
    const hits = DEV_LOCATIONS.filter(
      (l) => l.label.toLowerCase().includes(q) || l.id.includes(q.replace(/\s+/g, '-')),
    );
  ...
  async reverse(point: GeoPoint): Promise<Place | null> {
    ...
    for (const l of DEV_LOCATIONS) {
      const d =
        (l.point.latitude - point.latitude) ** 2 + (l.point.longitude - point.longitude) ** 2;
      if (d < bestD) {
        bestD = d;
        best = l;
      }
...
export class FixtureTimezoneProvider implements TimezoneProvider {
  readonly meta: ProviderMeta = {
    ...FIXTURE_META,
    id: 'fixture-timezone',
    name: 'Development fixture time zones',
  };

  async lookup(point: GeoPoint): Promise<string | null> {
    for (const l of DEV_LOCATIONS) {
      if (
        Math.abs(l.point.latitude - point.latitude) < 5 &&
        Math.abs(l.point.longitude - point.longitude) < 5
      )
        return l.timezone;
    }
