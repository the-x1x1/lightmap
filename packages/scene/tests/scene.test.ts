...
import { FIXTURE_CAPABILITIES, FixtureWeatherProvider, OPEN_METEO_CAPABILITIES } from '@lightmap/weather';
...
import { defaultCamera, focalLengthForFov, frameCoordinates, horizontalFovDeg, isInFrame, lightingGeometry, relativeBearing } from '../src/camera.ts';
...
const envGood: EnvironmentState = { terrainAvailable: true, terrainProviderId: 'reearth-mapterhorn-terrain', basemapProviderId: 'xyz-imagery', basemapDetail: 'street', buildingsAvailable: false, groundElevationM: 2, attributions: [], fixtureMode: false };
const envCoarse: EnvironmentState = { ...envGood, basemapProviderId: 'natural-earth-ii', basemapDetail: 'coarse' };
const envFlat: EnvironmentState = { ...envCoarse, terrainAvailable: false, terrainProviderId: 'ellipsoid' };
...
describe('buildSceneState — Kailua Beach, 31 May 2026, 12:30 (acceptance case)', () => {
  it('combines time, solar and weather correctly', () => {
    ...
    expect(s.localTime).toMatchObject({ date: '2026-05-31', time: '12:30', offsetMinutes: -600, zoneAbbreviation: 'HST' });
  ...
  it('uses the forecast frame inside the horizon and labels it a forecast', async () => {
    ...
    const series = await new FixtureWeatherProvider({ pattern: 'overcast' }).getForecast(21.397, -157.727, new Date('2026-05-31T00:00:00Z'), new Date('2026-06-01T00:00:00Z'));
    const s = buildSceneState(inputs({ now, weather: { capabilities: FIXTURE_CAPABILITIES, frames: series.frames, providerFailed: false } }));
  ...
  it('lets the user pin a scenario inside the forecast window and says so', async () => {
    const now = new Date('2026-05-30T00:00:00Z');
    const series = await new FixtureWeatherProvider({ pattern: 'overcast' }).getForecast(21.397, -157.727, new Date('2026-05-31T00:00:00Z'), new Date('2026-06-01T00:00:00Z'));
    const s = buildSceneState(inputs({ now, scenario: 'clear', forceScenario: true, weather: { capabilities: FIXTURE_CAPABILITIES, frames: series.frames, providerFailed: false } }));
  ...
  it('falls back to the scenario when the provider fails, without pretending', () => {
    const s = buildSceneState(inputs({ now: new Date('2026-05-30T00:00:00Z'), weather: { capabilities: OPEN_METEO_CAPABILITIES, frames: [], providerFailed: true } }));
    expect(s.atmosphere.mode).toBe('SCENARIO');
  ...
  it('scrubbing the clock changes solar state and colour temperature continuously', () => {
    const times = [6, 9, 12.5, 16, 18.5, 19.5, 21].map((h) => localSelectionToUtc({ year: 2026, month: 5, day: 31 }, h * 60, kailua.timeZone));
    ...
    expect(['nautical-twilight', 'astronomical-twilight', 'night']).toContain(states[6]!.solar.phase); // 21:00: sun ≈ −25°
    expect(states[4]!.atmosphere.colorTemperatureK).toBeLessThan(states[2]!.atmosphere.colorTemperatureK); // warmer late afternoon
  ...
  it('reuses cached day events for the same date and recomputes across midnight', () => {
    ...
    const b = buildSceneState(inputs({ dayEvents: a.dayEvents, utc: new Date(a.utc.getTime() + 3_600_000) }));
    expect(b.dayEvents).toBe(a.dayEvents);
    const c = buildSceneState(inputs({ dayEvents: a.dayEvents, utc: new Date(a.utc.getTime() + 24 * 3_600_000) }));
...
describe('confidence and source mode', () => {
  const horizon = { mode: 'SCENARIO' as const, leadHours: 700, reason: 'far', fetchWorthwhile: false, weatherConfidence: 'SCENARIO' as const };
  it('grades environment by terrain and detail', () => {
    expect(deriveConfidence({ astronomyInputsValid: true, environment: envGood, weather: horizon, realReference: false }).environment).toBe('MEDIUM');
    expect(deriveConfidence({ astronomyInputsValid: true, environment: { ...envGood, buildingsAvailable: true }, weather: horizon, realReference: false }).environment).toBe('HIGH');
    expect(deriveConfidence({ astronomyInputsValid: true, environment: envCoarse, weather: horizon, realReference: false }).environment).toBe('MEDIUM');
    expect(deriveConfidence({ astronomyInputsValid: true, environment: envFlat, weather: horizon, realReference: false }).environment).toBe('LOW');
  });
  it('maps to the three source labels', () => {
    expect(deriveSourceMode(deriveConfidence({ astronomyInputsValid: true, environment: envGood, weather: horizon, realReference: false }))).toBe('SIMULATED_LIGHTING');
    expect(deriveSourceMode(deriveConfidence({ astronomyInputsValid: true, environment: envCoarse, weather: horizon, realReference: false }))).toBe('ESTIMATED_PREVIEW');
    expect(deriveSourceMode(deriveConfidence({ astronomyInputsValid: true, environment: envFlat, weather: horizon, realReference: true }))).toBe('REAL_REFERENCE');
  });
  it('astronomy confidence drops only with invalid inputs', () => {
    expect(deriveConfidence({ astronomyInputsValid: false, environment: envGood, weather: horizon, realReference: false }).astronomy).toBe('LOW');
  });
});
