...
function oneOf<T extends string>(
  ...
  issues.push({ key, severity: 'error', message: `must be one of ${allowed.join(', ')} (got "${v}")` });
  return fallback;
}
...
export function parseEnv(raw: RawEnv): EnvResult {
  ...
  const NODE_ENV = oneOf(raw, 'NODE_ENV', ['development', 'test', 'production'] as const, 'development', issues);
  ...
  const env: Env = {
    ...
    WEATHER_PROVIDER: oneOf(raw, 'WEATHER_PROVIDER', ['open-meteo', 'fixture'] as const, 'open-meteo', issues),
    ...
    GEOCODER_PROVIDER: oneOf(raw, 'GEOCODER_PROVIDER', ['nominatim', 'fixture'] as const, 'nominatim', issues),
    GEOCODER_USER_AGENT: str(raw, 'GEOCODER_USER_AGENT') ?? 'LightMap-dev (unset contact)',
    TERRAIN_PROVIDER: oneOf(raw, 'TERRAIN_PROVIDER', ['reearth', 'cesium-ion', 'ellipsoid'] as const, 'reearth', issues),
    CESIUM_ION_TOKEN: str(raw, 'CESIUM_ION_TOKEN'),
    IMAGERY_PROVIDER: oneOf(raw, 'IMAGERY_PROVIDER', ['natural-earth', 'cesium-ion', 'xyz'] as const, 'natural-earth', issues),
    ...
    REFERENCE_IMAGERY_PROVIDER: oneOf(raw, 'REFERENCE_IMAGERY_PROVIDER', ['none'] as const, 'none', issues),
  ...
  if (env.AUTH_SECRET !== undefined && env.AUTH_SECRET.length < 32) {
    issues.push({ key: 'AUTH_SECRET', severity: isProd ? 'error' : 'warning', message: 'should be at least 32 characters (openssl rand -base64 32)' });
  }
  if (isProd) {
    for (const key of ['DATABASE_URL', 'AUTH_SECRET', 'AUTH_URL'] as const) {
      if (env[key] === undefined) issues.push({ key, severity: 'error', message: 'required in production' });
    }
    if (env.AUTH_DEV_LOGIN) issues.push({ key: 'AUTH_DEV_LOGIN', severity: 'error', message: 'must be off in production' });
    if (env.WEATHER_PROVIDER === 'fixture') issues.push({ key: 'WEATHER_PROVIDER', severity: 'error', message: 'fixture weather is not allowed in production (plan §33)' });
    if (env.GEOCODER_PROVIDER === 'fixture') issues.push({ key: 'GEOCODER_PROVIDER', severity: 'error', message: 'fixture geocoder is not allowed in production' });
    if (env.GEOCODER_PROVIDER === 'nominatim') issues.push({ key: 'GEOCODER_PROVIDER', severity: 'warning', message: 'Nominatim usage policy forbids heavy/commercial use; switch to a contracted geocoder before launch (docs/DATA_SOURCES_AND_LICENSING.md)' });
    if (env.WEATHER_PROVIDER === 'open-meteo' && env.OPEN_METEO_API_KEY === undefined) issues.push({ key: 'OPEN_METEO_API_KEY', severity: 'warning', message: 'Open-Meteo requires a paid API key for commercial use' });
    if (env.EMAIL_SERVER === undefined && env.AUTH_GOOGLE_ID === undefined) issues.push({ key: 'EMAIL_SERVER', severity: 'error', message: 'production needs at least one real sign-in method (email server or Google)' });
    if (env.STRIPE_SECRET_KEY !== undefined && env.STRIPE_WEBHOOK_SECRET === undefined) issues.push({ key: 'STRIPE_WEBHOOK_SECRET', severity: 'error', message: 'required when Stripe is configured' });
    if (env.LIGHTMAP_SHOW_DEV_BANNER) issues.push({ key: 'LIGHTMAP_SHOW_DEV_BANNER', severity: 'warning', message: 'dev banner is on in production' });
  }
  if (env.TERRAIN_PROVIDER === 'cesium-ion' && env.CESIUM_ION_TOKEN === undefined) {
    issues.push({ key: 'CESIUM_ION_TOKEN', severity: 'error', message: 'required when TERRAIN_PROVIDER=cesium-ion' });
  }
  if (env.IMAGERY_PROVIDER === 'cesium-ion' && env.CESIUM_ION_TOKEN === undefined) {
    issues.push({ key: 'CESIUM_ION_TOKEN', severity: 'error', message: 'required when IMAGERY_PROVIDER=cesium-ion' });
  }
  if (env.IMAGERY_PROVIDER === 'xyz') {
    if (env.IMAGERY_XYZ_URL === undefined) issues.push({ key: 'IMAGERY_XYZ_URL', severity: 'error', message: 'required when IMAGERY_PROVIDER=xyz' });
    if (env.IMAGERY_XYZ_ATTRIBUTION === undefined) issues.push({ key: 'IMAGERY_XYZ_ATTRIBUTION', severity: 'error', message: 'attribution is mandatory for every imagery source (plan §14)' });
  }
  if (!Number.isInteger(env.IMAGERY_XYZ_MAX_ZOOM) || env.IMAGERY_XYZ_MAX_ZOOM < 0 || env.IMAGERY_XYZ_MAX_ZOOM > 24) {
    issues.push({ key: 'IMAGERY_XYZ_MAX_ZOOM', severity: 'error', message: 'must be an integer 0–24' });
  }
  if (env.STRIPE_SECRET_KEY !== undefined && isProd && env.STRIPE_SECRET_KEY.startsWith('sk_test_')) {
    issues.push({ key: 'STRIPE_SECRET_KEY', severity: 'error', message: 'test-mode key in production' });
  }
