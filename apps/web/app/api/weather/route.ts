...
export async function GET(req: Request) {
  try {
    ...
    const civil = parseCivilDate(str(params, 'date', { required: true, pattern: /^\d{4}-\d{2}-\d{2}$/ })!);
    if (!civil) throw new HttpError(400, 'bad_request', 'date must be a valid YYYY-MM-DD');
    const tz = str(params, 'tz', { maxLength: 64 }) ?? 'Etc/UTC';
    if (!isValidTimeZone(tz)) throw new HttpError(400, 'bad_request', 'tz must be an IANA time zone');
    const { start, end } = localDayBounds(civil, tz);
    const caps = s.weather.getCapabilities();
    const decision = decideWeatherMode(new Date((start.getTime() + end.getTime()) / 2), new Date(), caps);
    ...
    const cacheKey = forecastCacheKey(caps.providerId, cell, `${civil.year}-${String(civil.month).padStart(2, '0')}-${String(civil.day).padStart(2, '0')}`);
    ...
    if (!series) {
      ...
      series = decision.mode === 'RECENT_PAST' ? await s.weather.getHistorical(clat, clng, start, end) : await s.weather.getForecast(clat, clng, start, end);
    ...
    const body: WeatherResponse = { providerId: series.providerId, mode: decision.mode, frames: series.frames, issuedAt: series.issuedAt, fetchedAt: series.fetchedAt, cached, attribution: caps.attribution };
    ...
    return errorResponse(new HttpError(502, 'weather_unavailable', 'Live forecast unavailable right now — showing your selected scenario.'));
  }
}
