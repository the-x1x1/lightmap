...
export async function POST() {
  try {
    ...
    if (!sub?.providerCustomerId) throw new HttpError(404, 'no_customer', 'No billing account yet — start a subscription first.');
    const { url } = await s.billing.createPortalSession(sub.providerCustomerId, `${s.env.NEXT_PUBLIC_APP_URL}/account`);
    return json({ url });
  } catch (e) {
