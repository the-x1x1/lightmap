...
export async function GET(req: Request) {
  try {
    ...
    const date = parseCivilDate(str(params, 'date', { required: true, pattern: /^\d{4}-\d{2}-\d{2}$/ })!);
    ...
    for (const [k, v] of Object.entries(ev)) if (v instanceof Date || v === null) events[k] = v ? v.toISOString() : null;
    const body: SolarDayResponse = { date: ev.date, timeZone: ev.timeZone, events, polar: ev.polar, notes: ev.notes, daylightMinutes: ev.daylightMinutes };
