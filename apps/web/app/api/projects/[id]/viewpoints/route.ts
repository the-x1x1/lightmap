...
export async function POST(req: Request, { params }: Params) {
  try {
    ...
    const [inProject, total] = await Promise.all([repo.countInProject(ctx.user.id, projectId), repo.countTotal(ctx.user.id)]);
    const decision = can(ctx.entitlements, 'saved_viewpoints', { viewpointCountInProject: inProject, viewpointCountTotal: total });
    ...
    const dateDecision = can(ctx.entitlements, 'future_date_planning', { targetDate: civil, today });
    ...
    return json({ viewpoint: viewpointDto({ ...vp, snapshot: snapshot ? { thumbnailDataUrl: snapshot.thumbnailDataUrl ?? null } : null }) }, { status: 201 });
  } catch (e) {
...
function isComplete(i: Partial<ViewpointInput>): i is ViewpointInput {
  return ['label', 'latitude', 'longitude', 'timezone', 'headingDeg', 'pitchDeg', 'fieldOfViewDeg', 'selectedDatetimeUtc', 'weatherMode', 'previewSourceType'].every((k) => (i as Record<string, unknown>)[k] !== undefined);
}
