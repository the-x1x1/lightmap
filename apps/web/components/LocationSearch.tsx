...
export function LocationSearch({
  ...
  useEffect(() => {
    ...
    if (coords) {
      setResults([
        {
          label: `Coordinates ${coords.latitude.toFixed(5)}, ${coords.longitude.toFixed(5)}`,
          point: coords,
  ...
  async function choose(p: Place) {
    ...
    setLocation({
      point: p.point,
      timeZone: usePlannerStore.getState().location?.timeZone ?? deviceZone,
      label: p.label,
      source: p.sourceId === 'coordinates' ? 'coordinates' : 'search',
    });
    try {
      const r = await api.get<ReverseResponse>(
        `/api/location/reverse?lat=${p.point.latitude.toFixed(5)}&lng=${p.point.longitude.toFixed(5)}`,
      );
      setLocation({
        point: { ...p.point, ...(r.elevationM !== null ? { elevationM: r.elevationM } : {}) },
        timeZone: r.timeZone,
        label: p.label,
        source: p.sourceId === 'coordinates' ? 'coordinates' : 'search',
      });
      ...
      (pos) => {
        ...
        void choose({
          ...
          point: {
            ...
            ...(pos.coords.altitude !== null ? { elevationM: pos.coords.altitude } : {}),
          },
          sourceId: 'device',
        });
      },
    ...
    <div className={cx('relative', className)}>
      <div className="flex items-center gap-2">
        ...
        <Button
          ...
          title="Use current location (asks permission)"
        >
          <svg
            width="18"
            height="18"
