...
export function Paywall({ reason, compact }: { reason?: string; compact?: boolean }) {
  ...
  const pro = PLANS.pro;
  if (!account.signedIn)
    return <SignInPrompt reason={reason ?? `${brand.billing.proName} needs an account.`} />;
  const configured = caps.data?.billingConfigured ?? false;
  return (
    <div
      className={cx(
        'rounded-[var(--lm-radius)] border border-[var(--lm-sun)]/30 bg-[color:rgba(245,179,66,0.08)] p-4',
        compact && 'p-3',
      )}
      data-testid="paywall"
    >
