...
export function AttributionFooter({
  ...
  for (const a of caps?.providers.attributions ?? [])
    items.set(a.id, {
      text: a.attribution,
      ...(a.attributionUrl ? { url: a.attributionUrl } : {}),
    });
  if (scene?.atmosphere.providerAttribution)
    items.set('weather', { text: scene.atmosphere.providerAttribution });
  if (items.size === 0) return null;
  return (
    <footer
      className="border-t border-[var(--lm-panel-border)] px-4 py-2 text-[10px] leading-relaxed text-[var(--lm-text-faint)]"
