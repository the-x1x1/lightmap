...
export function DevBanner({ caps }: { caps: CapabilitiesResponse | null }) {
  ...
  if (caps.providers.basemap.detailLevel === 'coarse') parts.push('coarse basemap (no imagery key)');
  if (caps.providers.terrain.kind === 'ellipsoid') parts.push('no terrain');
  return (
    <div role="status" className="pointer-events-none fixed inset-x-0 top-0 z-40 flex justify-center px-2 pt-1">
      <span className="rounded-b-md bg-[color:rgba(245,179,66,0.9)] px-2 py-0.5 text-[11px] font-medium text-[#1a1200]" data-testid="dev-banner">
