...
export function ForecastBadge({ scene, loading }: { scene: SceneState; loading?: boolean }) {
  ...
  switch (scene.atmosphere.mode) {
    case 'FORECAST':
      return (
        <Badge
          tone="ok"
          icon="◉"
          title={scene.confidence.notes.weather}
    ...
    case 'RECENT_PAST':
      return (
        <Badge tone="ok" icon="◉" title={scene.confidence.notes.weather}>
          Recent conditions
        </Badge>
      );
    case 'EXTENDED_FORECAST':
      return (
        <Badge tone="warn" icon="◔" title={scene.confidence.notes.weather}>
          Extended · low confidence
        </Badge>
      );
    case 'SCENARIO':
    case 'PAST':
