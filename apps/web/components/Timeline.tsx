...
export function dayMarkers(ev: DayEvents): Marker[] {
  const m = (key: string, d: Date | null, label: string, short: string, tone: Marker['tone']): Marker | null => {
...
export function dayGradient(ev: DayEvents): string {
  ...
  const pct = (d: Date | null) => (d ? `${((minutesOf(d, ev.dayStart)! / total) * 100).toFixed(2)}%` : null);
  if (ev.polar === 'midnight-sun') return 'linear-gradient(90deg, #6aa7e6, #8dbef0 50%, #6aa7e6)';
  if (ev.polar === 'polar-night') return ev.dawn ? `linear-gradient(90deg, #0c1230, #23305e ${pct(ev.dawn)}, #3a4f8a ${pct(ev.solarNoon) ?? '50%'}, #23305e ${pct(ev.civilDusk)}, #0c1230)` : 'linear-gradient(90deg, #0c1230, #141b3a, #0c1230)';
  const stops = [
    ['#0c1230', '0%'],
    ['#2b3a72', pct(ev.dawn)],
...
export function Timeline({ dayEvents, timeZone, phase, className }: TimelineProps) {
  ...
  const id = useId();
  const total = dayEvents ? Math.round((dayEvents.dayEnd.getTime() - dayEvents.dayStart.getTime()) / 60_000) : 1440;
  const markers = useMemo(() => (dayEvents ? dayMarkers(dayEvents) : []), [dayEvents]);
  const gradient = useMemo(() => (dayEvents ? dayGradient(dayEvents) : 'rgba(255,255,255,0.15)'), [dayEvents]);
  ...
  return (
    <div className={cx('w-full', className)} data-testid="timeline">
      <div className="mb-1 flex items-baseline justify-between">
        <label htmlFor={id} className="text-xs uppercase tracking-wide text-[var(--lm-text-muted)]">Time</label>
        <output htmlFor={id} className="font-mono text-2xl tabular-nums" aria-live="off" data-testid="timeline-time">{timeLabel}</output>
      </div>
      <div className="relative">
        <div aria-hidden className="pointer-events-none absolute left-0 right-0 top-1/2 h-1.5 -translate-y-1/2 rounded-full opacity-90" style={{ background: gradient }} />
        <input
          id={id}
          type="range"
          ...
          onKeyDown={(e) => {
            ...
            if (e.key === 'PageUp') { e.preventDefault(); setMinutes(minutes + 60); }
            else if (e.key === 'PageDown') { e.preventDefault(); setMinutes(minutes - 60); }
            else if (e.key === 'Home' && dayEvents?.sunrise) { e.preventDefault(); setMinutes(minutesOf(dayEvents.sunrise, dayEvents.dayStart)!); }
            else if (e.key === 'End' && dayEvents?.sunset) { e.preventDefault(); setMinutes(minutesOf(dayEvents.sunset, dayEvents.dayStart)!); }
          }}
          aria-valuetext={`${timeLabel}${phase ? `, ${phase.replace('-', ' ')}` : ''}`}
          aria-describedby={`${id}-desc`}
          data-testid="timeline-range"
        />
        <p id={`${id}-desc`} className="sr-only">Drag to scrub through the day. Arrow keys move one minute, Page Up and Page Down one hour, Home jumps to sunrise, End to sunset.</p>
          ...
          {markers.map((mk) => {
            ...
            return (
              <button
                ...
                className={cx('absolute -translate-x-1/2 text-[10px] leading-tight', mk.tone === 'sun' ? 'text-[var(--lm-sun)]' : mk.tone === 'twilight' ? 'text-[color:#9dbcff]' : 'text-[var(--lm-text-faint)]')}
                style={{ left }}
              >
      ...
      {dayEvents?.notes.length ? <p className="mt-1 text-xs text-[var(--lm-text-muted)]">{dayEvents.notes[0]}</p> : null}
    </div>
  );
}

export function DayEventMarkers({ dayEvents, timeZone }: { dayEvents: DayEvents; timeZone: string }) {
