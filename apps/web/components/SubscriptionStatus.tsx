  ...
  return (
    <div className="space-y-2 text-sm" data-testid="subscription-status">
      <div className="flex items-center gap-2">
        ...
        <Badge tone={snap.effectivePlan === 'free' ? 'neutral' : 'sun'}>
          {snap.planName}
          {snap.grace ? ' · payment issue' : ''}
        </Badge>
      </div>
      {snap.accessEndsAt ? (
        <p className="text-xs text-[var(--lm-text-muted)]">
          {sub?.cancelAtPeriodEnd ? 'Cancels' : 'Renews'}{' '}
          {new Date(snap.accessEndsAt).toLocaleDateString()}
        </p>
      ) : null}
      {snap.grace ? (
        <p className="text-xs text-[color:#ffd27a]">
          Your last payment failed. Access continues for a few days while Stripe retries — update
          your card in the portal.
        </p>
      ) : null}
      {sub?.hasCustomer ? (
