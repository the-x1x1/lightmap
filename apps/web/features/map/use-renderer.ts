...
export function useRenderer(opts: UseRendererOptions): RendererHandle {
  const [state, setState] = useState<RendererHandle>({
    mode: 'loading',
    error: null,
    capabilities: null,
    qualityLabel: '—',
  ...
  useEffect(() => {
    ...
    if (mode === 'OVERLAY') {
      setState((s) => ({
        ...
        capabilities: caps,
        error:
          'This browser has no WebGL2, so the 3D preview is unavailable. The map overlay still shows sun direction and light.',
      }));
      return;
    }
    let cancelled = false;
      ...
      try {
        ...
        const controller = new SceneController(host, {
          aspect: () =>
            opts.container.current
              ? opts.container.current.clientWidth /
                Math.max(1, opts.container.current.clientHeight)
              : 16 / 9,
        });
        ...
        const emitQuality = () => {
          ...
          onQualityRef.current({
            ...
            terrainScreenSpaceError: q.terrainScreenSpaceError,
        ...
        setState({
          ...
          error: null,
          capabilities: caps,
          qualityLabel: governor.quality.label,
          stats: host.stats(),
          controller,
          host,
        });
        ...
        setState((s) => ({
          ...
          mode: 'OVERLAY',
          capabilities: caps,
          error: 'The 3D preview could not start on this device. Showing the map overlay instead.',
        }));
      }
    })();
    return () => {
