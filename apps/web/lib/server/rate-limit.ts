...
export function checkBurst(key: string, limitPerMinute: number, now = Date.now()): void {
  ...
  if (arr.length >= limitPerMinute) throw new HttpError(429, 'rate_limited', 'Too many requests — slow down a little.');
  arr.push(now);
  windows.set(key, arr);
...
export async function chargeBudget(key: string, resource: BudgetResource, plan: 'anonymous' | 'free' | 'pro', amount = 1): Promise<void> {
  ...
  if (count > limit) throw new HttpError(429, 'budget_exhausted', `Daily ${resource} budget reached for this ${plan === 'anonymous' ? 'device' : 'plan'}. It resets at midnight UTC.`);
}
