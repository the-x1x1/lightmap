...
export async function requestContext(): Promise<RequestContext> {
  ...
  if (!id || !db) return { user: null, entitlements: deriveEntitlements(null), subscription: null, planForBudget: 'anonymous' };
  const subscription = await subscriptionsRepo(db.db).forUser(id);
  const entitlements = deriveEntitlements(
    subscription ? { planKey: subscription.planKey as 'free' | 'pro' | 'studio', status: subscription.status as never, periodEnd: subscription.periodEnd?.toISOString() ?? null, cancelAtPeriodEnd: subscription.cancelAtPeriodEnd } : null,
  );
  return {
...
export async function requireUser(): Promise<RequestContext & { user: RequestUser }> {
  ...
  if (!ctx.user) throw new HttpError(401, 'unauthenticated', 'Sign in to save projects and viewpoints.');
  return ctx as RequestContext & { user: RequestUser };
}

export function requireDb() {
  const { db } = getServices();
  if (!db) throw new HttpError(503, 'no_database', 'Saving is unavailable: the server has no database configured.');
  return db.db;
}
