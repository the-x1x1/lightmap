...
const allowlist = JSON.parse(readFileSync(join(root, 'scripts', 'license-allowlist.json'), 'utf8')) as Allowlist;
...
function licenseOf(p: PkgJson): string {
  ...
  if (Array.isArray(p.licenses) && p.licenses.length) return p.licenses.map((l) => l.type ?? '?').join(' OR ');
  return 'UNKNOWN';
}
...
function isPermissive(expr: string): boolean {
  ...
  if (/\bAND\b/.test(cleaned)) return cleaned.split(/\bAND\b/).every((part) => isPermissive(part.trim()));
  return cleaned.split(/\bOR\b/).some((part) => allowlist.permissive.includes(part.trim().replace(/\+$/, '')));
}
...
for (const r of roots) {
  ...
  const dirs = r.endsWith('.pnpm') ? readdirSync(r).map((d) => join(r, d, 'node_modules')).flatMap((nm) => (existsSync(nm) ? [...walk(nm)] : [])) : [...walk(r)];
  for (const dir of dirs) {
    ...
    seen.set(key, { version: pkg.version ?? '?', license: licenseOf(pkg), url: repo.replace(/^git\+/, '').replace(/\.git$/, '') });
  }
}
